...
const RatingComponent: React.FC<{ userId: string; jobId: string; onSubmit: (rating: number, feedback: string) => void }> = ({ userId, jobId, onSubmit }) => {
  ...
  if (!loading) {
    ...
    <div style={{border: "1px solid", width: "250px"}}>
      <div style={{border: "1px solid", width: "40px"}}>
        <img src={loader} />
      </div>
    </div> )
  }
  ...
  return (
    <div style={{border: "0px solid", width: "95%", margin:"auto", marginTop: "10px"}} className="flex flex-col items-center p-4">
      ...
      {isRated ? (
        <div style={{fontStyle: "italic", border: "0px solid", borderRadius: "15px", marginTop: "14px", padding: "5px", background: "grey"}}>"{feedback}"</div>
      ) : (
